...
def correlate_logs(sec, sysmon, fw, files, yocto=None):
    """Run core detectors and, when supplied, the optional Yocto detector."""
    all_alerts = []
    ...
    if yocto is not None and not yocto.empty:
        all_alerts += yocto_endpoint.detect(yocto)
